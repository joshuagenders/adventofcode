...
rotate_90 = lambda two_d_array: list(map(list, zip(*list(reversed(two_d_array)))))
...
for rotation in range(4):
    for x, line in enumerate(lines):
        ...
        for y, v in enumerate(line):
            tree_height = int(v)
            if tree_height > eye_line:
                visibility[x][y] = 1
            eye_line = max(eye_line, tree_height)
    lines = rotate_90(lines)
    visibility = rotate_90(visibility)

visible_trees = sum([sum(line) for line in visibility])
print(visible_trees)
