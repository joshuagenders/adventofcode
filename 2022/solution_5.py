...
input_groups = raw_input.split('\n\n')
stacks_input = input_groups[0]
instructions = input_groups[1]

stack_lines = stacks_input.splitlines()[:-1]
stacks = list(
    map(list,
        zip(
            *list(
                reversed(
                    list(
                        map(lambda line: [line[i:i+4] for i in range(0, len(line), 4)], stack_lines)
                    )
                )
            )
        )
