...
const functional = () => {
  const elfWithMostCalories = Math.max(
    ...inputRaw
      .split("\n")
      .reduce(
        (accumulator, value) =>
          value
            ? [
                ...
                accumulator[accumulator.length - 1] + parseInt(value),
              ]
            : [...accumulator, 0],
        [0]
      )
...
const functional2 = () => {
  ...
  const elfWithMostCalories = Math.max(
    ...inputRaw
      .split("\n\n")
      .map((elf_items) => elf_items.split("\n").map(toInt))
      .map((elf_item_ints) => elf_item_ints.reduce(sum))
  );
